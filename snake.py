...
Label(window, text='Project_AI_M.Haris.Zaman_452133', font='arial 20 bold').pack(side=BOTTOM)  # Footer
...
class Obstacle:
    ...
    def spawn_obstacle(self):
        # Randomly choose the position of the cross obstacle
        x = random.randint(0, (GAME_WIDTH / SPACE_SIZE) - 1) * SPACE_SIZE
        y = random.randint(0, (GAME_HEIGHT / SPACE_SIZE) - 1) * SPACE_SIZE
        ...
        for coord in self.coordinates:
            square = canvas.create_rectangle(coord[0], coord[1], coord[0] + SPACE_SIZE, coord[1] + SPACE_SIZE,
...
def check_collisions(snake, obstacle):
    ...
    # Check if snake collides with the obstacle
...
def restart_game():
    global snake, pc_snake, obstacle, food, score
    canvas.delete(ALL)
    score = 0
    label.config(text='Score:{}'.format(score))
    snake = Snake()
